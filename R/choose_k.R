...
analyzek <- function(filename) {
  ...
  y <- data.matrix(y)
  results = array(0, dim = c(116, (nrow(y) - rollingwindow - 1))) # where to store the results
...
#Pilot version of choosing k====
#using a subset 5 subjects
setwd("~/UoB/Code/Data_Atlanta fMRI")
filenames <- list.files(path = "~/UoB/Code/Data_Atlanta fMRI",  
                        ("csv"))

filenames <- filenames[1:length(filenames)][1:5]
rollingwindow = 120
