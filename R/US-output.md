...
``` r
load("~/Downloads/LRPS-main/data/USdata.rda")
X <- USdata$X
Y <- USdata$Y
X <- as.matrix(X)
Y <- as.matrix(Y)
...
```
    ...
    ## Prediction error for OLS: 0.9254462
...
```
    ...
    ## Prediction error for RRR: 0.9160992

``` r
cat("Optimal rank for RRR:", optimal_k_rrr)
```

    ## Optimal rank for RRR: 1
...
```
    ...
    ## Prediction error for lrps: 0.9107826

``` r
cat("Optimal rank for LRPS:", optimal_k_lrps)
```

    ## Optimal rank for LRPS: 2
