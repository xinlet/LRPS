...
``` r
load("~/Downloads/LRPS-main/data/geno.rda")
X <- geno$X
Y <- geno$Y
X <- as.matrix(X)
Y <- as.matrix(Y)
...
```
    ...
    ## Prediction error for OLS: 0.4813219
...
```
    ...
    ## Prediction error for RRR: 0.4176113

``` r
cat("Optimal rank for RRR:", optimal_k_rrr)
```

    ## Optimal rank for RRR: 2
...
```
    ...
    ## Prediction error for lrps: 0.4143034
