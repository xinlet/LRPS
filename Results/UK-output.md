...
``` r
load("~/Downloads/LRPS-main/data/UKdata.rda")
X <- UKdata$X
Y <- UKdata$Y
X <- as.matrix(X)
Y <- as.matrix(Y)
...
```
    ...
    ## Prediction error for OLS: 0.7511158
...
```
    ...
    ## Prediction error for RRR: 0.7493672
...
```
    ...
    ## Prediction error for lrps: 0.7377472

``` r
cat("Optimal rank for LRPS:", optimal_k_lrps)
```

    ## Optimal rank for LRPS: 5
