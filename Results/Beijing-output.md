...
``` r
load("~/Downloads/LRPS-main/data/Beijingdata.rda")
X <- Beijingdata$X
Y <- Beijingdata$Y
X <- as.matrix(X)
Y <- as.matrix(Y)
...
```
    ...
    ## Prediction error for OLS: 1.174533
...
```
    ...
    ## Prediction error for RRR: 1.154324

``` r
cat("Optimal rank for RRR:", optimal_k_rrr)
```

    ## Optimal rank for RRR: 6
...
```
    ...
    ## Prediction error for lrps: 1.145383

``` r
cat("Optimal rank for LRPS:", optimal_k_lrps)
```

    ## Optimal rank for LRPS: 12
